...
如果多个内存访问在访问同一个地址、并且至少其中一个为写访问，我们称这种情况为数据竞争。
...
游泳馆有有限个手环，在进入游泳馆时，给每个人发一个手环（持有 Semaphore）；只有持有手环的人才能进入游泳馆（互斥）；当储物柜被占满时，我们无法给新来的人发手环了，于是他们只能等待（等待）；离开游泳馆的人要还回手环（释放）。

我们可以发现，互斥锁 mutex 是 Semaphore 中 N = 1 的特殊情况。

信号量有两种主要行为：请求和释放。当信号量有剩余时，则请求的结果是持有信号量直到释放；当信号量没有剩余时，则请求的结果是等待，直到有多余的信号量释放出来则变为持有信号量。
...
- Producer (生产数据)：如果缓冲区有空位，放入；否则等待（阻塞）

- Consumer (消费数据)：如果缓冲区有数据，取走；否则等待（阻塞）
